...
import { contasPagar, formatCurrency } from "@/lib/mock-data";

export default function ContasPagar() {
  const [filtroStatus, setFiltroStatus] = useState<string>("todos");
  const [busca, setBusca] = useState("");

  const filtrados = contasPagar.filter((c) => {
            ...
            <tbody>
              {filtrados.map((conta) => (
                <tr key={conta.id} className="cursor-pointer">
                  ...
                  <td className="text-muted-foreground">{conta.fornecedor}</td>
                  <td className="text-muted-foreground">{conta.categoria}</td>
                  <td className="text-muted-foreground">{conta.centroCusto}</td>
